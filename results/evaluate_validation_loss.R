...
get_eval_loss<-function(model, version, len, mountpoint = "/Volumes/ru87tow/") {
  ...
  n_epochs <- as.numeric(strsplit(version, "_(?!.*_)", perl=TRUE)[[1]][2])
  seconds_epoch <- (as.numeric(substr(time, 1,2))*3600 + 
    as.numeric(substr(time, 4,5))*60 + as.numeric(substr(time, 7,10)))/n_epochs
  ...
  for (i in 1:length(logfile$log_history)){
    if("eval_loss" %in% names(logfile$log_history[[i]])){
      ...
      log <- rbind(log, data.frame(step = step, eval_loss = eval_loss, time = round(seconds)))
      seconds <- seconds + seconds_epoch
    }
  }
  eval_loss_final <- read.table(path_eval_loss_final)[1,2]
  log <- rbind(log, data.frame(step = log$step[nrow(log)]+log$step[1], 
                               eval_loss = eval_loss_final, time = round(seconds)))
  return(log)
}
